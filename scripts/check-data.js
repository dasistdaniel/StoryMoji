...
for (const cat of cardData.categories) {
  for (const lang of LANGS) {
    if (!cat.label?.[lang])
      errors.push(`Category "${cat.id}" missing ${lang} label`);
  }
}
...
for (const card of cardData.cards) {
  ...
  for (const lang of LANGS) {
    if (!card.term?.[lang])
      errors.push(`Card "${card.id}" missing ${lang} term`);
  }

  perCategory.set(card.category, (perCategory.get(card.category) || 0) + 1);
}

// Categories that are complete as-is and are not expected to reach 12 cards.
const FIXED_SIZE_CATEGORIES = new Set(["numbers"]);

for (const id of categoryIds) {
  const n = perCategory.get(id) || 0;
  if (n === 0) errors.push(`Category "${id}" has no cards`);
  else if (n < 12 && !FIXED_SIZE_CATEGORIES.has(id)) {
    warnings.push(`Category "${id}" has only ${n} cards (< 12)`);
  }
}
...
for (const prompt of promptData.prompts) {
  ...
  for (const lang of LANGS) {
    ...
    for (const token of tokens) {
      if (token !== "{card}") {
        errors.push(
          `Prompt "${prompt.id}" (${lang}) has unknown token ${token}`
        );
      }
    }
  }
